...
function buildTitleAnalysis(title) {
  ...
  platformLimits.forEach((platform) => {
    ...
    row.innerHTML = `
      <div class="platform-title-label">${platform.name} <span class="limit">(${platform.limit})</span></div>
      <div class="platform-title-bar-wrap">
        <div class="platform-title-bar ${barClass}" style="width:${pct}%"></div>
      </div>
      <div class="platform-title-status ${statusClass}">${statusText}</div>
    `;
